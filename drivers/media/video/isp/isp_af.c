...
static int isp_af_check_params(struct isp_af_device *af,
...
static void isp_af_config_registers(struct isp_af_device *af)
{
	...
	spin_lock_irqsave(af->lock, irqflags);

	isp_reg_writel(isp, af->buf_next->iommu_addr, OMAP3_ISP_IOMEM_H3A,
		       ISPH3A_AFBUFST);

	if (!af->update) {
		spin_unlock_irqrestore(af->lock, irqflags);
		return;
	}

	/* Configure Hardware Registers */
	pax1 |= af->config.paxel_config.width << AF_PAXW_SHIFT;
	/* Set height in AFPAX1 */
	pax1 |= af->config.paxel_config.height;
	isp_reg_writel(isp, pax1, OMAP3_ISP_IOMEM_H3A, ISPH3A_AFPAX1);

	/* Configure AFPAX2 Register */
	/* Set Line Increment in AFPAX2 Register */
	pax2 |= af->config.paxel_config.line_incr << AF_LINE_INCR_SHIFT;
	/* Set Vertical Count */
	pax2 |= af->config.paxel_config.vt_cnt << AF_VT_COUNT_SHIFT;
	/* Set Horizontal Count */
	pax2 |= af->config.paxel_config.hz_cnt;
	isp_reg_writel(isp, pax2, OMAP3_ISP_IOMEM_H3A, ISPH3A_AFPAX2);

	/* Configure PAXSTART Register */
	/*Configure Horizontal Start */
	paxstart |= af->config.paxel_config.hz_start << AF_HZ_START_SHIFT;
	/* Configure Vertical Start */
	paxstart |= af->config.paxel_config.vt_start;
	isp_reg_writel(isp, paxstart, OMAP3_ISP_IOMEM_H3A,
		       ISPH3A_AFPAXSTART);

	/*SetIIRSH Register */
	isp_reg_writel(isp, af->config.iir_config.hz_start_pos,
	...
	for (index = 0; index <= 8; index += 2) {
		...
		coef = 0;
		coef |= af->config.iir_config.coeff_set0[index];
		coef |= af->config.iir_config.coeff_set0[index + 1] <<
			AF_COEF_SHIFT;
		isp_reg_writel(isp, coef, OMAP3_ISP_IOMEM_H3A,
			       base_coef_set0);
		base_coef_set0 += AFCOEF_OFFSET;

		/*Set IIR Filter1 Coefficients */
		coef = 0;
		coef |= af->config.iir_config.coeff_set1[index];
		coef |= af->config.iir_config.coeff_set1[index + 1] <<
			AF_COEF_SHIFT;
		isp_reg_writel(isp, coef, OMAP3_ISP_IOMEM_H3A,
			       base_coef_set1);
	...
	isp_reg_writel(isp, af->config.iir_config.coeff_set0[10],
		       OMAP3_ISP_IOMEM_H3A, ISPH3A_AFCOEF0010);
	/* set AFCOEF1010 Register */
	isp_reg_writel(isp, af->config.iir_config.coeff_set1[10],
		       OMAP3_ISP_IOMEM_H3A, ISPH3A_AFCOEF1010);

	/* PCR Register */
	/* Set Accumulator Mode */
	if (af->config.mode == ACCUMULATOR_PEAK)
		pcr |= FVMODE;
	/* Set A-law */
	if (af->config.alaw_enable == H3A_AF_ALAW_ENABLE)
		pcr |= AF_ALAW_EN;
	/* Set RGB Position */
	pcr |= af->config.rgb_pos << AF_RGBPOS_SHIFT;
	/* HMF Configurations */
	if (af->config.hmf_config.enable == H3A_AF_HMF_ENABLE) {
		/* Enable HMF */
		pcr |= AF_MED_EN;
		/* Set Median Threshold */
		pcr |= af->config.hmf_config.threshold << AF_MED_TH_SHIFT;
	}
	/* Set PCR Register */
	...
	af->update = 0;
	ispstat_bufs_set_size(&af->stat, af->buf_size);
...
static void isp_af_update_params(struct isp_af_device *af,
				 struct af_configuration *afconfig)
{
	int update = 0;
	int index;
	...
	/* alaw */
	if (af->config.alaw_enable != afconfig->alaw_enable) {
		update = 1;
		goto out;
	}

	/* hmf */
	if (af->config.hmf_config.enable != afconfig->hmf_config.enable) {
		update = 1;
		goto out;
	}
	if (af->config.hmf_config.threshold !=
	...
	/* rgbpos */
	if (af->config.rgb_pos != afconfig->rgb_pos) {
		update = 1;
		goto out;
	}

	/* iir */
	if (af->config.iir_config.hz_start_pos !=
					afconfig->iir_config.hz_start_pos) {
		update = 1;
		goto out;
	}
	for (index = 0; index < AF_NUMBER_OF_COEF; index++) {
		if (af->config.iir_config.coeff_set0[index] !=
				afconfig->iir_config.coeff_set0[index]) {
			update = 1;
			goto out;
		}
		if (af->config.iir_config.coeff_set1[index] !=
	...
	/* paxel */
	if ((af->config.paxel_config.width !=
				afconfig->paxel_config.width) ||
	    (af->config.paxel_config.height !=
				afconfig->paxel_config.height) ||
	    (af->config.paxel_config.hz_start !=
				afconfig->paxel_config.hz_start) ||
	    (af->config.paxel_config.vt_start !=
				afconfig->paxel_config.vt_start) ||
	    (af->config.paxel_config.hz_cnt !=
				afconfig->paxel_config.hz_cnt) ||
	    (af->config.paxel_config.line_incr !=
	...
	/* af_mode */
	if (af->config.mode != afconfig->mode) {
		update = 1;
		goto out;
	}

	af->config.af_config = afconfig->af_config;

out:
	if (update) {
		memcpy(&af->config, afconfig, sizeof(*afconfig));
		af->update = 1;
	}
}

void isp_af_try_enable(struct isp_af_device *af)
{
	unsigned long irqflags;
	...
	spin_lock_irqsave(af->lock, irqflags);
	if (unlikely(!af->enabled && af->config.af_config)) {
		af->update = 1;
		af->buf_next = ispstat_buf_next(&af->stat);
		spin_unlock_irqrestore(af->lock, irqflags);
		isp_af_config_registers(af);
		isp_af_enable(af, 1);
	} else
...
int isp_af_config(struct isp_af_device *af,
		  struct af_configuration *afconfig)
{
	struct isp_device *isp = to_isp_device(af);
	int result;
	...
	if (!afconfig) {
		dev_dbg(isp->dev, "af: Null argument in configuration.\n");
		return -EINVAL;
	}

	/* Check Parameters */
	spin_lock_irqsave(af->lock, irqflags);
	result = isp_af_check_params(af, afconfig);
	spin_unlock_irqrestore(af->lock, irqflags);
	if (result) {
		dev_dbg(isp->dev, "af: wrong configure params received.\n");
		return result;
	}
	...
	result = ispstat_bufs_alloc(&af->stat, buf_size, 0);
	if (result)
		return result;

	spin_lock_irqsave(af->lock, irqflags);
	af->buf_size = buf_size;
	isp_af_update_params(af, afconfig);
	spin_unlock_irqrestore(af->lock, irqflags);
...
int isp_af_buf_process(struct isp_af_device *af)
{
	if (likely(!af->buf_err && af->config.af_config)) {
		int ret;

		ret = ispstat_buf_queue(&af->stat);
		af->buf_next = ispstat_buf_next(&af->stat);
		return ret;
	} else {
		af->buf_err = 0;
...
void isp_af_enable(struct isp_af_device *af, int enable)
{
	unsigned long irqflags;

	spin_lock_irqsave(af->lock, irqflags);

	if (!af->config.af_config && enable) {
		spin_unlock_irqrestore(af->lock, irqflags);
		return;
	}

	__isp_af_enable(af, enable);
	af->enabled = enable;
...
void isp_af_suspend(struct isp_af_device *af)
{
	unsigned long irqflags;

	spin_lock_irqsave(af->lock, irqflags);
	if (af->enabled)
		__isp_af_enable(af, 0);
	spin_unlock_irqrestore(af->lock, irqflags);
}

/* Function to Resume AF Engine */
void isp_af_resume(struct isp_af_device *af)
{
	unsigned long irqflags;

	spin_lock_irqsave(af->lock, irqflags);
	if (af->enabled)
		__isp_af_enable(af, 1);
	spin_unlock_irqrestore(af->lock, irqflags);
}
...
int isp_af_init(struct isp_device *isp)
{
	struct isp_af_device *af = &isp->isp_af;

	af->lock = &isp->h3a_lock;
	ispstat_init(isp, "AF", &af->stat, H3A_MAX_BUFF,
		     MAX_FRAME_COUNT);
