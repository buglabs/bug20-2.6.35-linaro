...
static int bug_spi_uart_gpio_setup(struct spi_device *spi, unsigned gpio, unsigned ngpio, void *context)
{
	...
	printk(KERN_INFO "spi_uart_gpio: Setting up gpios...%d\n", gpio);
	//bug_display_init();
...
void gen_gpio_settings(void)
{
	...
	omap_mux_init_gpio(64, OMAP_PIN_INPUT);
...
*/
	r =   gpio_request(186, "sigma_asic_enable");
	if (r) {
		printk(KERN_ERR "gen_gpio: failed to get sigma_asic_enable...\n");
		return;
	}
	gpio_direction_output(186, 1);

	r =   gpio_request(170, "asic_reset");
	if (r) {
		printk(KERN_ERR "gen_gpio: failed to get asic_reset...\n");
		return;
	}
