/*
 * drivers/bmi/pims/comcast/bmi_comcast_media.c
 *
...
#include <linux/bmi/bmi_comcast_media.h>
#include <linux/fs.h>
...
#define BMI_COMCAST_MEDIA_VERSION  "0.1.0.1"
static int major;		// control device major

// BMI device ID table
static struct bmi_device_id bmi_comcast_media_tbl[] = 
{ 
	{ .match_flags = BMI_DEVICE_ID_MATCH_VENDOR | BMI_DEVICE_ID_MATCH_PRODUCT, 
	  .vendor   = BMI_VENDOR_BUG_LABS, 
	  .product  = BMI_PRODUCT_COMCAST_MEDIA, 
	  .revision = BMI_ANY, 
	}, 
	{ 0, },				      /* terminate list */
};
MODULE_DEVICE_TABLE(bmi, bmi_comcast_media_tbl);

int	bmi_comcast_media_probe(struct bmi_device *bdev);
void 	bmi_comcast_media_remove(struct bmi_device *bdev);

// BMI driver structure
static struct bmi_driver bmi_comcast_media_driver = 
{
	.name = "bmi_comcast_media", 
	.id_table = bmi_comcast_media_tbl, 
	.probe   = bmi_comcast_media_probe, 
	.remove  = bmi_comcast_media_remove, 
	};


struct bmi_comcast_media {
...
static int comcast_media_ioexp_gpio_setup(struct i2c_client *client,
...
static int comcast_media_ioexp_gpio_teardown(struct i2c_client *client,
					     unsigned gpio, unsigned ngpio,
					     void *context)
{
  return 0;
...
static struct pca953x_platform_data comcast_media_ioexp_data = {
	.gpio_base  = GPIO_NUM,
	.setup	    = comcast_media_ioexp_gpio_setup,
	.teardown   = comcast_media_ioexp_gpio_teardown,
};

#define BMI_IOX_I2C_ADDRESS	0x26	// 7-bit address
static struct i2c_board_info comcast_media_i2c_boardinfo = {
	I2C_BOARD_INFO("pca9555", BMI_IOX_I2C_ADDRESS),
	//.irq = gpio_to_irq(63),
	.platform_data = &comcast_media_ioexp_data,
};
...
{
	struct bmi_comcast_media *med = dev_get_drvdata(dev);
	int value=0, addr=-1;
...
static void comcast_media_set_bootstraps(unsigned int mode)
{
	int i, bit;
	// set bootstrap pins
	for(i=0; i<16; i++) {
...
static void init_gpios(struct bmi_comcast_media *med)
{
	...
	comcast_media_set_bootstraps(BMI_COMCAST_MEDIA_BOOT_SPI);
}

// deconfigure GPIOs
static void uninit_gpios(struct bmi_comcast_media *med)
{
 	int slot = med->bdev->slot->slotnum;
...
static int comcast_media_set_red_led(struct bmi_device *bdev, int on) 
{
	return gpio_direction_output(GPIO_LED_RED, on ? 1 : 0);
}

static int comcast_media_set_green_led(struct bmi_device *bdev, int on) 
{
	return gpio_direction_output(GPIO_LED_GREEN, on ? 1 : 0);
}

static int comcast_media_vixs_reboot(struct bmi_device *bdev, unsigned int mode)
{
	...
	if(mode == BMI_COMCAST_MEDIA_BOOT_SPI) {
		bmi_slot_gpio_direction_out(slot, GPIO_SPI_SEL,  0);
	}

	comcast_media_set_bootstraps(mode);
...
{	
	struct bmi_comcast_media *med;

	med = container_of (inode->i_cdev, struct bmi_comcast_media, cdev);
...
{	
	struct bmi_comcast_media *med;
	med = (struct bmi_comcast_media *)(file->private_data);
	med->open_flag = 0;
	return 0;
}
...
{	
	...
	case BMI_COMCAST_MEDIA_SUSPEND:
		med->bdev->dev.bus->pm->suspend(&med->bdev->dev);
		break;
	case BMI_COMCAST_MEDIA_RESUME:
		med->bdev->dev.bus->pm->resume(&med->bdev->dev);
		break;
	case BMI_COMCAST_MEDIA_RLEDOFF:
		comcast_media_set_red_led(med->bdev, 0);
		break;
	case BMI_COMCAST_MEDIA_RLEDON:
		comcast_media_set_red_led(med->bdev, 1);
		break;
	case BMI_COMCAST_MEDIA_GLEDOFF:
		comcast_media_set_green_led(med->bdev, 0);
		break;
	case BMI_COMCAST_MEDIA_GLEDON:
		comcast_media_set_green_led(med->bdev, 1);
		break;
	case BMI_COMCAST_MEDIA_VIXS_REBOOT:
		comcast_media_vixs_reboot(med->bdev, (unsigned int) arg);
		break;
	case BMI_COMCAST_MEDIA_SPI_FLASH_SEL:
		bmi_slot_gpio_direction_out(slot, GPIO_SPI_SEL,  arg ? 1 : 0);
		break;
	case BMI_COMCAST_MEDIA_VIXS_RESET:
...
{	
	...
	med->iox  = i2c_new_device(bdev->slot->adap, &comcast_media_i2c_boardinfo);
	if(!med->iox) {
	...
	for(i=GPIO_NUM; i<16+GPIO_NUM; i++) {
		ret |= gpio_request(i, "comcast_media");
...
//	printk(KERN_ERR "%s: %s\n", __func__, mtd->name);
	...
	comcast_media_vixs_reboot(bdev, BMI_COMCAST_MEDIA_BOOT_SPI);
	return 0;
...
{	
	int i;
	struct class *bmi_class;
	struct bmi_comcast_media *med = bmi_device_get_drvdata (bdev);
	int slot = bdev->slot->slotnum;
...
static __init int bmi_comcast_media_init(void)
{	
	dev_t	dev_id;
	int	retval;

	// alloc char driver with 4 minor numbers
	retval = alloc_chrdev_region (&dev_id, 0, 4, "BMI Comcast Media Driver"); 
	if (retval) {
		return -ENODEV;
	}

	major = MAJOR(dev_id);
	retval = bmi_register_driver (&bmi_comcast_media_driver);   
	if (retval) {
...
static void __exit bmi_comcast_media_cleanup(void)
{	
	dev_t dev_id;
...
module_init(bmi_comcast_media_init);
module_exit(bmi_comcast_media_cleanup);

MODULE_AUTHOR("Lane Brooks");
MODULE_DESCRIPTION("BMI Comcast Media Driver");
MODULE_LICENSE("GPL");
