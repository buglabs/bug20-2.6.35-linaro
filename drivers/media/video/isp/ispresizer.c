				       ...
				       const struct resizer_ratio *ratio)
{
	...
	if (res->input == RESIZER_INPUT_MEMORY) {
		...
		res->crop_offset = (crop_rect->top * input->width +
				    crop_rect->left) * 2;
		/*
		 ...
		 */
		ispresizer_set_start(res, crop_rect->left * 2, crop_rect->top);
...
static void resizer_configure(struct isp_res_device *res)
{
	...
	ispresizer_calc_ratios(res, &res->crop, outformat, &res->ratio);
	ispresizer_set_crop_params(res, &res->crop, informat, outformat,
				   &res->ratio);
}
...
void ispresizer_isr(struct isp_res_device *res)
{
	...
	if (res->applycrop) {
		...
		informat = __resizer_get_format(res, NULL, RESZ_PAD_SINK,
					      V4L2_SUBDEV_FORMAT_ACTIVE);
		ispresizer_set_crop_params(res, &res->crop, informat,
					   outformat, &res->ratio);
		res->applycrop = 0;
	}
			  ...
			  struct v4l2_subdev_pad_crop *crop)
{
	...
	if (crop->which == V4L2_SUBDEV_FORMAT_PROBE)
		crop->rect = *v4l2_subdev_get_probe_crop(fh, RESZ_PAD_SINK);
	else
		crop->rect = res->crop;
			  ...
			  struct v4l2_subdev_pad_crop *crop)
{
	...
	ispresizer_set_crop_params(res, &res->crop, format_sink, format_source,
				   &res->ratio);
			      ...
			      enum v4l2_subdev_format which)
{
	...
	*format = *fmt;
	...
	if (pad == RESZ_PAD_SINK) {
		...
		res->crop.left = 0;
		res->crop.top = 0;
		res->crop.width = fmt->width;
		res->crop.height = fmt->height;
