/*
 * drivers/bmi/pims/dect/bmi_dect.c
 *
...
#define BMI_DECT_VERSION  "0.1.0.1"
static int major;		// control device major

// BMI device ID table
static struct bmi_device_id bmi_dect_tbl[] = 
{ 
	{ .match_flags = BMI_DEVICE_ID_MATCH_VENDOR | BMI_DEVICE_ID_MATCH_PRODUCT, 
	  .vendor   = BMI_VENDOR_BUG_LABS, 
	  .product  = BMI_PRODUCT_DECT, 
	  .revision = BMI_ANY, 
	}, 
	{ 0, },				      /* terminate list */
};
MODULE_DEVICE_TABLE(bmi, bmi_dect_tbl);

int	bmi_dect_probe(struct bmi_device *bdev);
void 	bmi_dect_remove(struct bmi_device *bdev);

// BMI driver structure
static struct bmi_driver bmi_dect_driver = 
{
	.name = "bmi_dect", 
	.id_table = bmi_dect_tbl, 
	.probe   = bmi_dect_probe, 
	.remove  = bmi_dect_remove, 
	};
...
static void init_gpios(struct bmi_dect *dect)
{
...
// deconfigure GPIOs
static void uninit_gpios(struct bmi_dect *dect)
{
 	int slot = dect->bdev->slot->slotnum;
...
static void dect_set_red_led(struct bmi_device *bdev, int on) 
{
	int slot = bdev->slot->slotnum;
	bmi_slot_gpio_direction_out(slot, GPIO_RED_LED, on ? 1 : 0);
}

static void dect_set_green_led(struct bmi_device *bdev, int on) 
{
	int slot = bdev->slot->slotnum;
...
static int bmi_dect_program_init(struct bmi_dect *dect) {
	int ret  = 0;
	int slot = dect->bdev->slot->slotnum;
...
static int bmi_dect_program_uninit(struct bmi_dect *dect) {
	int slot = dect->bdev->slot->slotnum;
	bmi_slot_gpio_direction_out(slot, GPIO_IO_RESETB, 0);
	return 0;
}
...
{	
	struct bmi_dect *dect;

	dect = container_of (inode->i_cdev, struct bmi_dect, cdev);
...
{	
	struct bmi_dect *dect;
	dect = (struct bmi_dect *)(file->private_data);
	dect->open_flag = 0;
	return 0;
}
...
{	
	...
	case BMI_DECT_SUSPEND:
		dect->bdev->dev.bus->pm->suspend(&dect->bdev->dev);
		break;
	case BMI_DECT_RESUME:
		dect->bdev->dev.bus->pm->resume(&dect->bdev->dev);
		break;
	case BMI_DECT_RLEDOFF:
		dect_set_red_led(dect->bdev, 0);
		break;
	case BMI_DECT_RLEDON:
		dect_set_red_led(dect->bdev, 1);
		break;
	case BMI_DECT_GLEDOFF:
		dect_set_green_led(dect->bdev, 0);
		break;
	case BMI_DECT_GLEDON:
		dect_set_green_led(dect->bdev, 1);
		break;
	case BMI_DECT_PROGRAM_INIT:
		if(arg) {
...
{	
	struct class *bmi_class;
	struct bmi_dect *dect = bmi_device_get_drvdata (bdev);
	int slot = bdev->slot->slotnum;
...
static __init int bmi_dect_init(void)
{	
	dev_t	dev_id;
	int	retval;

	// alloc char driver with 4 minor numbers
	retval = alloc_chrdev_region (&dev_id, 0, 4, "BMI DECT Driver"); 
	if (retval) {
		return -ENODEV;
	}

	major = MAJOR(dev_id);
	retval = bmi_register_driver (&bmi_dect_driver);   
	if (retval) {
...
static void __exit bmi_dect_cleanup(void)
{	
	dev_t dev_id;
...
module_init(bmi_dect_init);
module_exit(bmi_dect_cleanup);

MODULE_AUTHOR("Lane Brooks");
MODULE_DESCRIPTION("BMI DECT Driver");
MODULE_LICENSE("GPL");
